...
// Import all images from all galleries for random selection
...
const Index: React.FC = () => {
  ...
  const [randomImage] = useState(() => {
    ...
    return allGalleryImages[randomIndex];
  });

  const handleClick = (e: React.MouseEvent) => {
    // Prevent navigation if clicking on the logo or enter button
  ...
  return (
    ...
    >
      ...
      {/* V&Co logo - white, links to home */}
      <Link
        to="/"
        onClick={(e) => e.stopPropagation()}
        className="absolute top-16 left-16 z-20 text-white text-4xl leading-none tracking-[-1.07px] font-bold hover:opacity-80 transition-opacity"
      >
        V&Co
      </Link>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-10 text-white">
        <h1 className="text-6xl tracking-[0.3em] font-medium max-md:text-4xl max-md:tracking-[0.2em] text-center">
          Veronica & Co
        </h1>
        <h2 className="text-sm tracking-[0.3em] mt-6 uppercase text-center">
        ...
        <Link
          to="/about"
          onClick={(e) => e.stopPropagation()}
          className="mt-16 text-white/60 hover:text-white/90 text-sm uppercase tracking-wider transition-colors duration-300 underline underline-offset-4"
        >
          Enter
        </Link>
      </div>
